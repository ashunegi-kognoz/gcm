...
def write_pfms(indices, total_length, pfms_file, lang1_code, lang2_code):
    ...
    # If PFMS file does not exist, assume uniform scores
    pfms_path = os.path.join(input_loc, pfms_file) if pfms_file else None

    if not pfms_file or not pfms_path or not os.path.exists(pfms_path):
        pfms_scores = ["0.0" for _ in range(total_length)]
    else:
        with open(pfms_path, "r") as f:
            lines = f.read().split("\n")
        for index in indices:
            if 0 <= index < len(lines):
                pfms_scores.append(lines[index])

    out_pfms = f"{lang1_code}-to-{lang2_code}_pfms.txt"
    with open(os.path.join(output_loc, out_pfms), "w") as f:
        f.write("\n".join(pfms_scores))
...
def fast_align(pfms_file, lang1_code, lang2_code, align_op_file, lang1_op_file, lang2_op_file):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        path_to_fast_align = "{}/fast_align-master/build".format(root_dir)
